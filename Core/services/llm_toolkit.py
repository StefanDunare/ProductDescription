...
class ChatManager:
    ...
    def _load_memory(self):
        ...
        if not self.memory_file:
             print("Running chat with no memory.")
             return
    ...
    async def send_message(self, user_message: str) -> str:
        ...
        print("Sending message to Gemini with memory... ")
        try:
...
async def remove_popups_and_overlays(page: Page):
    ...
    try:
        ...
        removed_count = await page.evaluate(f"""() => {{
            ...
            let count = 0;
            elements.forEach(el => {{
                // We add a check to make sure the element is still in the document
                // before trying to remove it, which is a good safety measure.
